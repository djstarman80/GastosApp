...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  List<Map<String, dynamic>> _getGastosDelMes(
    List<dynamic> gastos,
    ...
    for (final g in gastos) {
      ...
      if (g.esRecurrente) {
        ...
        if (_selectedYear < mesResumen.year ||
            (_selectedYear == mesResumen.year &&
                _selectedMonth >= mesResumen.month)) {
        ...
        if (fechaCierre != null) {
          ...
          int mesesTranscurridos =
              (_selectedYear - anioInicio) * 12 + (_selectedMonth - mesInicio);
        ...
        if (fechaCierre != null) {
          final mesResumen = CuotaUtils.calcularMesResumen(
            fechaGasto,
            fechaCierre,
          );
          if (mesResumen.year == _selectedYear &&
              mesResumen.month == _selectedMonth) {
            montoMostrar = g.monto;
            result.add({
              'gasto': g,
              'monto': montoMostrar,
          ...
          // Fallback: usar mes calendario si no hay fecha de cierre
          if (fechaGasto.year == _selectedYear &&
              fechaGasto.month == _selectedMonth) {
            montoMostrar = g.monto;
            result.add({
              'gasto': g,
              'monto': montoMostrar,
  ...
  Widget build(BuildContext context) {
    ...
    final monthGastos = _getGastosDelMes(
      gastoState.gastos,
      tarjetaState.tarjetas,
    );

    final totalMes = monthGastos.fold(
      0.0,
      (sum, g) => sum + (g['monto'] as double),
    );

    final gastosPorUsuario = <int, double>{};
    for (final g in monthGastos) {
      final gasto = g['gasto'];
      gastosPorUsuario[gasto.usuarioId] =
          (gastosPorUsuario[gasto.usuarioId] ?? 0) + (g['monto'] as double);
    }

    final gastosPorTarjeta = <int, double>{};
    for (final g in monthGastos) {
      final gasto = g['gasto'];
      gastosPorTarjeta[gasto.tarjetaId] =
          (gastosPorTarjeta[gasto.tarjetaId] ?? 0) + (g['monto'] as double);
    }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: _isLoading ? null : () => _showExportPdfDialog(context),
          ),
          IconButton(
            icon: Icon(
            ...
            onPressed: () {
              ref
                  ...
                  .setDarkMode(!ref.read(isDarkModeProvider));
            },
          ),
        ],
      ),
                              ...
                              children: [
                                ...
                                DropdownButton<String>(
                                  ...
                                  items: _meses
                                      ...
                                      .toList(),
                                  onChanged: (mes) => setState(
                                    () => _selectedMonth =
                                        _meses.indexOf(mes!) + 1,
                                  ),
                                ),
                                const SizedBox(width: 8),
                                DropdownButton<int>(
                      ...
                      ...gastosPorUsuario.entries.map((entry) {
                        ...
                        return Card(
                          child: ListTile(
                            ...
                            title: Text(
                              usuario?.nombre ?? 'Usuario ${entry.key}',
                            ),
                            trailing: Text(
                            ...
                            onTap: () => context.push(
                              '/user_expenses/${entry.key}?year=$_selectedYear&month=$_selectedMonth',
                            ),
                          ),
                        );
                      }),
                      ...
                      ...gastosPorTarjeta.entries.map((entry) {
                        ...
                        return Card(
                          child: ListTile(
                            ...
                            title: Text(
                              tarjeta?.nombre ?? 'Tarjeta ${entry.key}',
                            ),
                            trailing: Text(
                              CurrencyFormatter.format(entry.value),
                              style: const TextStyle(
                            ...
                            onTap: () => context.push(
                              '/tarjeta_expenses/${entry.key}?year=$_selectedYear&month=$_selectedMonth',
                            ),
                          ),
                        );
                      }),
  ...
  Future<void> _exportData() async {
    ...
    try {
      ...
      if (kIsWeb) {
        downloadJson(
          json,
          'backup_gastosapp_${DateTime.now().toIso8601String()}.json',
        );
      } else {
        ...
        final result = await FilePicker.platform.saveFile(
          ...
          fileName:
              'backup_gastosapp_${DateTime.now().millisecondsSinceEpoch}.json',
  ...
  void _showExportPdfDialog(BuildContext context) {
    showDialog(context: context, builder: (context) => const ExportPdfDialog());
  }
}
